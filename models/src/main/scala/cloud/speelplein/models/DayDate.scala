...
object DayDate {
  def createFromDayId(dayId: Day.Id): Try[DayDate] =
    Try(LocalDate.parse(dayId, DateTimeFormatter.ISO_LOCAL_DATE))
      .map(createFromLocalDate)
...
case class DayDate(day: Int, month: Int, year: Int) extends Ordered[DayDate] {
  ...
  override def compare(that: DayDate): Int =
    this.toLocalDate.compareTo(that.toLocalDate)
