...
class FiscalCertificateService @Inject()(
  ...
  def getFiscalCertificateSheet(year: Int)(
    ...
    for {
      ...
      FiscalCertificateCalculator(year, allDays, allChildren, allAttendances).calculateAll
  ...
  private case class FiscalCertificateCalculator(
    ...
    def calculateAll: Sheet = {
      ...
      val sheetDataRows = certRows.map(
        row =>
          Row().withCellValues(
            row.lastName,
            row.firstName,
    ...
    private def shiftIdToAmountPaidOnDay(
        dayId: Day.Id,
        shiftsAttended: Seq[Shift.Id]): Price = {
      val shifts = allDays
        .find(_.id == dayId)
        .get
        .entity
        .shifts
        .filter(x => shiftsAttended contains x.id)
      shifts.map(_.price).fold(Price(0, 0))(_ + _)
    }

    private def getTotalPricePaidForAttendances(
        attendances: Seq[DayAttendance]): Price = {
      val listOfPrices: Seq[Price] = attendances.map(x =>
        shiftIdToAmountPaidOnDay(x.day, x.shifts.map(_.shiftId)))
      val totalPrice = listOfPrices.fold(Price(0, 0))(_ + _)
      totalPrice
    }

    private def findAttendancesForChild(id: Child.Id): Seq[DayAttendance] =
      allAttendances.getOrElse(id, Seq.empty)

    private def child2certificateRow(child: Child,
                                     id: Child.Id): CertificateRow = {
      def dayId2Day(id: Day.Id): Day = allDays.filter(_.id == id).head.entity

      val attendances = findAttendancesForChild(id).filter(att =>
        getDayDate(att.day).year == year)

      CertificateRow(
        child.lastName,
        child.firstName,
        child.legacyAddress.street
          .map(_ + " ")
          .getOrElse("") + child.legacyAddress.number.getOrElse(""),
        child.legacyAddress.zipCode
          .map(_ + " ")
          .getOrElse("") + child.legacyAddress.city.getOrElse(""),
        child.birthDate
          .map(_.toLocalDate.format(DateTimeFormatter.ofPattern("dd/MM/yyyy")))
          .getOrElse(""),
    ...
    private def getDayDate(dayId: Day.Id): DayDate =
      DayDate.createFromDayId(dayId).get
  }
}
