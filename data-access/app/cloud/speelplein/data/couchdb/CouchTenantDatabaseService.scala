...
@Singleton
class CouchTenantDatabaseService @Inject()(
    wsClient: WSClient,
    couchdbConfig: CouchDbConfig)(implicit ec: ExecutionContext)
  ...
  override def create(db: DbName): Future[Res.Ok] =
    client.dbs.create(db.value).unsafeToFuture()

  override def details(db: DbName): Future[Res.DbInfo] =
    client.dbs.get(db.value).unsafeToFuture()

  override def drop(db: DbName): Future[Res.Ok] =
    client.dbs.delete(db.value).unsafeToFuture()

  override def designDocExists(dbName: DbName,
                               designName: String): Future[Option[String]] = {
    client
      .db(dbName.value, TypeMapping.empty)
      .design
      .get(designName)
      .unsafeToFuture()
      .map(t => Some(t._rev))
  ...
  override def createDesignDoc(db: DbName,
                               couchDesign: CouchDesign): Future[DocOk] = {
    client
      .db(db.value, TypeMapping.empty)
      .design
      .create(couchDesign)
      .unsafeToFuture()
  }

  override def startReplicationToRemote(
      db: DbName,
      target: CouchDbConfig): Future[JsValue] = {
    val url = target.uri / db.value
    val document = Json.toJson(ReplicationDocument(db.value, url))

    logger.info(
      "Starting replication local --> remote. Replication document to post: " + document)

    wsClient.url(couchdbConfig.uri / "_replicate").post(document).map(_.json)
  }

  override def startReplicationFromRemote(db: CouchDbConfig,
                                          target: DbName): Future[JsValue] = {
    val url = db.uri / target.value
    val document = Json.toJson(ReplicationDocument(url, target.value))

    logger.info(
      "Starting replication remote --> local. Replication document to post: " + document)
