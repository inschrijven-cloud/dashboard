  ...
  "ExportService#childSheet" should {
    "export a sheet with all children" in {
      ...
      val children = Seq(
        EntityWithId[Child.Id, Child](
          ...
          Child(
            ...
            ContactInfo(
              Seq(PhoneContact("phoneNumber", Some("kind"), Some("comment"))),
              Seq("aoeu@example.com")),
            None,
            Seq.empty,
            Some(DayDate(22, 12, 2016)),
        ...
        EntityWithId[Child.Id, Child](
          ...
          Child(
            ...
            ContactInfo(
              Seq(
                PhoneContact("phoneNumber", Some("kind"), Some("comment")),
                PhoneContact("phone2", None, None),
                PhoneContact("phone3", Some("landline"), None)
              ),
              Seq("aoeu@example.com", "secondary@example.com")
            ),
            None,
            Seq.empty,
            None,
      ...
      (childRepo
        .findAll(_: Tenant))
        .expects(*)
        .returns(Future.successful(children))
        ...
        .withRows(
          Seq(
            ...
            Row().withCellValues(
              ...
              "",
              "",
              "aoeu@example.com, secondary@example.com",
              "phoneNumber (comment) (kind), phone2, phone3 (landline)")
          )
        )
        .withColumns(
  ...
  "ExportService#crewSheet" should {
    "return a sheet with all crew members" in {
      ...
      val crew = Seq(
        EntityWithId[Crew.Id, Crew](
          ...
          Crew(
            "first",
            "last",
            Address(Some("street"), Some("number"), Some(6666), Some("city")),
            true,
            Some("bank"),
            ContactInfo(
              Seq(PhoneContact("phoneNumber", Some("kind"), Some("comment"))),
      ...
      (crewRepo
        .findAll(_: Tenant))
        .expects(*)
        .returning(Future.successful(crew))
