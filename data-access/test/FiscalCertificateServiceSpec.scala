...
class FiscalCertificateServiceSpec
  ...
  "Fiscal certificate service" should {
    "return a sheet with no date rows when there are no children" in {
      ...
      (childAttendanceService
        .findAll(_: Tenant))
        .expects(*)
        .returning(Future.successful(Map.empty))
        .once()

      val service = new FiscalCertificateService(childRepo,
                                                 dayService,
                                                 childAttendanceService,
                                                 global)
    ...
    "return a sheet with data for one child with one attendance" in {
      ...
      (childRepo
        .findAll(_: Tenant))
        .expects(*)
        .returning(Future.successful(children))

      val dayService = mock[DayService]
      (dayService
        .findAll(_: Tenant))
        .expects(*)
        .returning(Future.successful(days))
        .once()
      ...
      (childAttendanceService
        ...
        .returning(
          Future.successful(
            Map(
              "child1" -> Seq(
                DayAttendance("2016-03-07",
                              Seq(SingleAttendance("shift1", None, None))))
            )
          ))
        .once()
