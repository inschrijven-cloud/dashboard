...
class JwtVerifyActionSpec
  ...
  "JwtVerifyAction" should {
    "succeed when JWT is valid" in {
      val tenantRes = Await.result(
        tenantAction.refine(
          FakeRequest("GET", "/blah?tenant=example")
            .withHeaders(Headers(
      ...
      val jwtRes =
        Await.result(jwtVerifyAction.refine(tenantRes.right.value), 2.seconds)
    ...
    "set isGlobalSuperUser when user has the superuser role in the global tenant" in {
      val tenantRes = Await.result(
        tenantAction.refine(
          FakeRequest("GET", "/blah?tenant=example")
            .withHeaders(Headers(
