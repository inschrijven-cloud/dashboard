...
import cloud.speelplein.models.JsonFormats.{ crewFormat, crewWithIdWrites, entityWithIdReads }
import cloud.speelplein.dashboard.controllers.actions.{ DomainAction, JwtAuthorizationBuilder }
...
class CrewApiController @Inject() (
  ...
  def create: Action[EntityWithId[Crew.Id, Crew]] = action(crewCreate).async(parse.json(entityWithIdReads[Crew.Id, Crew])) { req =>
  ...
  def getById(id: Crew.Id): Action[AnyContent] = action(crewRetrieve).async { req =>
    crewRepository.findById(id)(req.tenant).map { crewOpt =>
      crewOpt.map(crew => Json.toJson(crew.entity)).map(Ok(_)).getOrElse(NotFound)
    }
  }

  def update(id: Crew.Id): Action[Crew] = action(crewUpdate).async(parse.json(crewFormat)) { crewReq =>
    crewRepository.update(id, crewReq.body)(crewReq.tenant).map(_ => updated(id))
  }

  def delete(id: Crew.Id): Action[AnyContent] = action(crewDelete).async { req =>
