...
class ContactPersonApiController @Inject() (
  ...
  def create: Action[EntityWithId[ContactPerson.Id, ContactPerson]] = (Action andThen domainAction).async(parse.json(entityWithIdReads[ContactPerson.Id, ContactPerson])) { req =>
    contactPersonRepository.insert(req.body.id, req.body.entity)(req.tenant).map(created)
  }

  def getById(id: ContactPerson.Id): Action[AnyContent] = (Action andThen domainAction).async { req =>
  ...
  def update(id: ContactPerson.Id): Action[ContactPerson] = (Action andThen domainAction).async(parse.json(JsonFormats.contactPersonFormat)) { req =>
    contactPersonRepository.update(id, req.body)(req.tenant).map(_ => updated(id))
  }

  def delete(id: ContactPerson.Id): Action[AnyContent] = (Action andThen domainAction).async { req =>
