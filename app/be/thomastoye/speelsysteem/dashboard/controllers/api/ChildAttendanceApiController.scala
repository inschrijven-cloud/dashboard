...
class ChildAttendanceApiController @Inject() (
  ...
  def numberOfChildAttendances: Action[AnyContent] = (Action andThen domainAction andThen jwtAuthorizationBuilder.authenticate(childAttendanceRetrieve)).async { req =>
  ...
  def getAttendancesForChild(id: Child.Id): Action[AnyContent] = (Action andThen domainAction andThen jwtAuthorizationBuilder.authenticate(childAttendanceRetrieve)).async { req =>
    childAttendancesService.findAttendancesForChild(id)(req.tenant).map(att => Ok(Json.toJson(att)))
  }

  def addAttendancesForChild(childId: Child.Id, dayId: Day.Id): Action[BindShiftIds] = (Action andThen domainAction andThen jwtAuthorizationBuilder.authenticate(childAttendanceCreate)).async(parse.json(bindShiftIdsReads)) { req =>
  ...
  def deleteAttendancesForChild(childId: Child.Id, dayId: Day.Id): Action[BindShiftIds] = (Action andThen domainAction andThen jwtAuthorizationBuilder.authenticate(childAttendanceDelete)).async(parse.json(bindShiftIdsReads)) { req =>
  ...
  def findAllPerChild: Action[AnyContent] = (Action andThen domainAction andThen jwtAuthorizationBuilder.authenticate(childAttendanceRetrieve)).async { req =>
    childAttendancesService.findAll(req.tenant).map(all => Ok(Json.toJson(all)))
  }

  def findAllPerDay: Action[AnyContent] = (Action andThen domainAction andThen jwtAuthorizationBuilder.authenticate(childAttendanceRetrieve)).async { req =>
  ...
  def findAllRaw: Action[AnyContent] = (Action andThen domainAction andThen jwtAuthorizationBuilder.authenticate(childAttendanceRetrieve)).async { req =>
