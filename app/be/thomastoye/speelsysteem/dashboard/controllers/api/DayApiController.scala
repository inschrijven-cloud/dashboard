...
class DayApiController @Inject() (
  ...
  def all: Action[AnyContent] = (Action andThen domainAction andThen jwtAuthorizationBuilder.authenticate(dayRetrieve)).async { req =>
    dayService.findAll(req.tenant).map(days => Ok(Json.toJson(days)))
  }

  def create: Action[Day] = (Action andThen domainAction andThen jwtAuthorizationBuilder.authenticate(dayCreate)).async(parse.json(dayFormat)) { req =>
    dayService.insert(req.body)(req.tenant).map(_ => Ok)
  }

  def getById(id: Day.Id): Action[AnyContent] = (Action andThen domainAction andThen jwtAuthorizationBuilder.authenticate(dayRetrieve)).async { req =>
    dayService.findById(id)(req.tenant).map { maybeDay =>
      maybeDay.map(dayWithId => Ok(Json.toJson(dayWithId))).getOrElse(NotFound)
    }
  }

  def update(id: Day.Id): Action[Day] = (Action andThen domainAction andThen jwtAuthorizationBuilder.authenticate(dayUpdate)).async(parse.json(dayFormat)) { req =>
