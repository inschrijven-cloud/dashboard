...
class CrewApiController @Inject() (
  ...
  def create: Action[EntityWithId[Crew.Id, Crew]] = (Action andThen domainAction andThen jwtAuthorizationBuilder.authenticatePermission("contactperson:create")).async(parse.json(entityWithIdReads[Crew.Id, Crew])) { req =>
  ...
  def getById(id: Crew.Id): Action[AnyContent] = (Action andThen domainAction andThen jwtAuthorizationBuilder.authenticatePermission("contactperson:retrieve")).async { req =>
    crewRepository.findById(id)(req.tenant).map { crewOpt =>
      crewOpt.map(crew => Json.toJson(crew.entity)).map(Ok(_)).getOrElse(NotFound)
    }
  }

  def update(id: Crew.Id): Action[Crew] = (Action andThen domainAction andThen jwtAuthorizationBuilder.authenticatePermission("contactperson:update")).async(parse.json(crewFormat)) { crewReq =>
    crewRepository.update(id, crewReq.body)(crewReq.tenant).map(_ => updated(id))
  }

  def delete(id: Crew.Id): Action[AnyContent] = (Action andThen domainAction andThen jwtAuthorizationBuilder.authenticatePermission("contactperson:delete")).async { req =>
