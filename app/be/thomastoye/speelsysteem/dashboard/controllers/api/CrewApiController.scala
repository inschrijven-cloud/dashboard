...
class CrewApiController @Inject() (
  ...
  def create: Action[EntityWithId[Crew.Id, Crew]] = (Action andThen domainAction andThen jwtAuthorizationBuilder.authenticatePermission(Permissions.Crew.create)).async(parse.json(entityWithIdReads[Crew.Id, Crew])) { req =>
    crewRepository.insert(req.body.id, req.body.entity)(req.tenant).map(created)
  }

  def all: Action[AnyContent] = (Action andThen domainAction andThen jwtAuthorizationBuilder.authenticatePermission(Permissions.Crew.retrieve)).async { req =>
    crewRepository.findAll(req.tenant).map(all => Ok(Json.toJson(all)))
  }

  def getById(id: Crew.Id): Action[AnyContent] = (Action andThen domainAction andThen jwtAuthorizationBuilder.authenticatePermission(Permissions.Crew.retrieve)).async { req =>
    crewRepository.findById(id)(req.tenant).map { crewOpt =>
      crewOpt.map(crew => Json.toJson(crew.entity)).map(Ok(_)).getOrElse(NotFound)
    }
  }

  def update(id: Crew.Id): Action[Crew] = (Action andThen domainAction andThen jwtAuthorizationBuilder.authenticatePermission(Permissions.Crew.update)).async(parse.json(crewFormat)) { crewReq =>
    crewRepository.update(id, crewReq.body)(crewReq.tenant).map(_ => updated(id))
  }

  def delete(id: Crew.Id): Action[AnyContent] = (Action andThen domainAction andThen jwtAuthorizationBuilder.authenticatePermission(Permissions.Crew.delete)).async { req =>
