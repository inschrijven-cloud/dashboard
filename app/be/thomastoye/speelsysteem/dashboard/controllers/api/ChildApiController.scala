...
class ChildApiController @Inject() (
  ...
  def all: Action[AnyContent] = (Action andThen domainAction andThen jwtAuthorizationBuilder.authenticatePermission("child:retrieve")).async { req =>
    childRepository.findAll(req.tenant).map(all => Ok(Json.toJson(all)))
  }

  def create: Action[EntityWithId[Child.Id, Child]] = (Action andThen domainAction andThen jwtAuthorizationBuilder.authenticatePermission("child:create"))
  ...
  def getById(id: Child.Id): Action[AnyContent] = (Action andThen domainAction andThen jwtAuthorizationBuilder.authenticatePermission("child:retrieve")).async { req =>
  ...
  def update(id: Child.Id): Action[Child] = (Action andThen domainAction andThen jwtAuthorizationBuilder.authenticatePermission("child:update")).async(parse.json(childFormat)) { req =>
    childRepository.update(id, req.body)(req.tenant).map(_ => updated(id))
  }

  def delete(id: Child.Id): Action[AnyContent] = (Action andThen domainAction andThen jwtAuthorizationBuilder.authenticatePermission("child:delete")).async { req =>
    childRepository.delete(id)(req.tenant).map(_ => Ok)
  }

  def merge(retiredId: Child.Id, absorpedIntoId: Child.Id): Action[AnyContent] = (Action andThen domainAction andThen jwtAuthorizationBuilder.authenticatePermission("child:merge")).async { req =>
