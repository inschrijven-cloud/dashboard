...
import be.thomastoye.speelsysteem.models.{ Auth0AppMetadata, Tenant, TenantMetadata }
...
trait JwtAuthorizationBuilder {
  def authenticate(permissions: Seq[Permission] = Seq.empty, roles: Seq[String] = Seq.empty): ActionFunction[DomainRequest, JwtRequest]

  def authenticate(permission: Permission): ActionFunction[DomainRequest, JwtRequest] = authenticate(Seq(permission), Seq.empty)
  ...
  def authenticate(permissions: Seq[Permission] = Seq.empty, roles: Seq[String] = Seq.empty): ActionFunction[DomainRequest, JwtRequest] = {
    jwtVerifyAction andThen new ActionRefiner[JwtRequest, JwtRequest] {
      ...
      def refine[A](request: JwtRequest[A]): Future[Either[Result, JwtRequest[A]]] = {
        if (request.tenantData.permissions.intersect(permissions.map(_.id)).nonEmpty || request.tenantData.roles.intersect(roles).nonEmpty) {
